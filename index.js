...
witbot.hears('performance', 0.5, function (bot, message, outcome) {
	...
	if (!outcome.entities.datetime || outcome.entities.datetime.length === 0) {
		console.log(outcome.entities.metric)
	    bot.reply(message, 'I\'d love to give you the answer. But I need you to specify a start and end date!')
	    return
	}	
	...
	if (dateType === "interval"){
  	...
  	analytics.get(metric, startDate, endDate, function (error, msg2) {
	    if (error) {
